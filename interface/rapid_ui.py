    ...
    with st.spinner("جاري التشخيص..."):
        try:
            ...
            if response.status_code == 200:
                ...
                if "error" in result:
                    st.error(f"خطأ من الخادم: {result['error']}")
                else:
                    hfe_prob = result.get('hfe_probability', 0)
                    ari_prob = result.get('ari_probability', 1 - hfe_prob)
                    ...
                    diagnosis = result.get('primary_diagnosis', 'غير محدد')
                    recommendation = result.get('recommendation', 'لا توجد')
                    ...
                    st.caption(f"🕐 تم التشخيص: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            else:
